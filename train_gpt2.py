...
class Block(nn.Module):
    ...
    def forward(self, x, v1, x0, v_weighted_skip, block_mask):
        ...
        x1, v1, v = self.attn(F.rms_norm(x, (x.size(-1),)), v1, v_weighted_skip, block_mask)
        x = x + x1
...
class DistributedDataLoader:
    ...
    def next_batch(self):
        ...
        buf = torch.from_numpy(buf.astype(np.int32)).to(dtype=torch.long).pin_memory()
        x = buf[:-1] # inputs
        y = buf[1:] # targets
...
val_loader = DistributedDataLoader(args.input_val_bin, B, T_val, ddp_rank, ddp_world_size)
if master_process:
